...
// * A file whose usage line documents `source <file>` is treated as a sourced library and is
...
// A file meant to be sourced rather than executed has no command line to
// document, so the --help rule does not apply to it. We only grant the
// exemption when the script's own usage line says it must be sourced, so an
// ordinary CLI that simply forgot --help still fails.
function isSourcedLibrary(source) {
  const head = source.split('\n').slice(0, 60).join('\n');
  return /^\s*(?:#|\*)?\s*Usage:.*\bsource\s+\S+/im.test(head);
}
...
function checkHelp(source) {
  ...
  if (isSourcedLibrary(source)) {
    return {
      status: 'ok',
      note: 'Sourced library; its usage line documents `source`, so --help does not apply.',
    };
  }
  return {
    status: 'fail',
