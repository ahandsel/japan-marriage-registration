...
* **`--help`.** Looks for `--help` anywhere in the source. A script whose usage line documents `source <file>` is treated as a sourced library and is exempt, because it has no command line of its own. A script that parses `--help` but routes it elsewhere may need a manual check.
