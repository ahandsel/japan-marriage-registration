...
* **`config-private.yaml` is the local file users fill with their real PII.**
  ...
  With `-t`, it first resolves the template, so a typo never creates a config named after a template that does not exist, and it inserts a `template: <variant>` key after the header when the file has none, so the file still selects its own form when it is later passed by path; a key that names a different form is kept and reported with a ⚠️.
