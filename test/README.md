...
| [repo.test.mjs][]   | Repository hygiene: nothing private is tracked and `.gitignore` covers the private names, the tracked configs match the sample's key set and keep their documented blanks, `package.json` scripts are sorted, helper script versions match their history, workflows are keyed to `main` and pin SHAs, and the docs match the code they describe.                                                    |
